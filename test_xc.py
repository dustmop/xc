...
class XCTests(unittest.TestCase):
  ...
  def test_flag_parse(self):
    (vals, flags, errs) = xc.collect_values(['-x', '37'])
    self.assertEqual(errs, [])
    self.assertEqual(flags, set(['hex']))
    self.assertEqual(vals, [37])
    (vals, flags, errs) = xc.collect_values(['-d', '0x1337'])
    self.assertEqual(errs, [])
    self.assertEqual(flags, set(['dec']))
    self.assertEqual(vals, [4919])
  ...
  def test_parse_error(self):
    (vals, flags, errs) = xc.collect_values(['123', '+', 'abc'])
    self.assertEqual(errs, [{'kind': 'parse',
                             'message': 'Failed to parse "abc"',
                             'detail': 'abc'}])
    self.assertEqual(flags, set())
